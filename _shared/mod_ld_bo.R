...
prepare_candidates <- function(ca0) {
  ca0 <- as.data.frame(ca0)
  
  # Esperem EXACTE: chr,pos_ini,pos_end,id_hit,classe (com has dit)
  stopifnot(all(c("chr","pos_ini","pos_end","id_hit","classe") %in% names(ca0)))
  
  # Canonical LD_Inspector schema:
  # chr, position, rsid, classe
  out <- data.frame(
    chr      = suppressWarnings(as.integer(ca0$chr)),
    position = suppressWarnings(as.integer(ca0$pos_ini)),
    rsid     = as.character(ca0$id_hit),
    classe   = as.character(ca0$classe),
    stringsAsFactors = FALSE
  )
  
  out <- out[is.finite(out$chr) & is.finite(out$position) & nzchar(out$rsid), , drop = FALSE]
  out <- out[!duplicated(paste(out$chr, out$position, out$rsid, sep="|")), , drop = FALSE]
...
available_pops_dir <- function(ld_pops_dir) {
  d <- tryCatch(normalizePath(ld_pops_dir, winslash = "/", mustWork = FALSE), error = function(e) ld_pops_dir)
  if (is.null(d) || !nzchar(d) || !dir.exists(d)) return(character(0))
  ff <- list.files(d, pattern = "\\.txt$", full.names = FALSE)
  pops <- sub("\\.txt$", "", ff, ignore.case = TRUE)
  sort(unique(pops[nzchar(pops)]))
}

read_keep_file_dir <- function(pop, ld_pops_dir) {
  d <- tryCatch(normalizePath(ld_pops_dir, winslash = "/", mustWork = FALSE), error = function(e) ld_pops_dir)
  if (is.null(d) || !nzchar(d) || !dir.exists(d)) stop("POP keep-files directory not found: ", ld_pops_dir)
  
  f <- file.path(d, paste0(pop, ".txt"))
  if (!file.exists(f)) stop("Missing keep file: ", f)
      ...
      shiny::sidebarPanel(
        ...
        shiny::wellPanel(
          ...
          shiny::textInput(
            ...
            value = Sys.getenv(
              "PLINK_BIN",
              unset = "/Volumes/DISK1TB/Inspector_app_slaves/Inspector_resources/software/plink19"
            )
          ),
          ...
          shiny::textInput(
            ...
            value = Sys.getenv(
              "PLINK_BFILE",
              unset = "/Volumes/DISK1TB/Inspector_app_slaves/Inspector_resources/LD_resources/Merged_FULL_SET_hg38_hgdp.wgs_10000G3_ko07_MAF0.05"
            )
          ),
          ...
          shiny::textInput(
            ...
            value = Sys.getenv(
              "LD_POPS_DIR",
              unset = "/Volumes/DISK1TB/Inspector_app_slaves/Inspector_resources/LD_resources/POP"
            )
          )
        ),
...
# SERVER
# --------------------------------------------------------------------------
ld_module_server <- function(id, clusters_r, candidates_r) {
  moduleServer(id, function(input, output, session) {
    ...
    # pop UI from ld_pops_dir
    output$ld_pop_ui <- shiny::renderUI({
      pops <- available_pops_dir(input$ld_pops_dir %||% "")
      if (length(pops) == 0) {
        return(shiny::tags$div(
          style = "color:#b00020; font-weight:600;",
          paste0("No .txt keep files found in: ", input$ld_pops_dir %||% "(empty)")
        ))
      }
      sel <- if (!is.null(input$ld_pop) && input$ld_pop %in% pops) input$ld_pop else {
        if ("EUR" %in% pops) "EUR" else pops[1]
      }
      shiny::selectInput(session$ns("ld_pop"), "Population (keep file)", choices = pops, selected = sel)
    })
    ...
    shiny::observeEvent(list(clusters_df(), input$ld_chr), {
      ...
      cl_chr <- cl %>%
        dplyr::filter(suppressWarnings(as.integer(chr)) == chr_sel) %>%
    ...
    selected_cluster <- shiny::reactive({
      ...
      one <- cl %>%
        dplyr::filter(
          as.integer(chr) == as.integer(input$ld_chr),
    ...
    shiny::observeEvent(input$run_ld_cluster, {
      ...
      tryCatch({
        ...
        append_log(paste0("[LD] ERROR: ", e$message))
    ...
    output$ld_fl_dt <- DT::renderDT({
      ...
      if (is.null(fl) || !nrow(fl)) {
        return(DT::datatable(data.frame(Message="Run LD to populate SNP list."),
      ...
      DT::datatable(show, rownames = FALSE, options = list(pageLength = 15, scrollX = TRUE))
    })
    ...
    output$ld_pairs_dt <- DT::renderDT({
      ...
      if (is.null(lp) || !nrow(lp) || is.null(fl) || !nrow(fl)) {
        return(DT::datatable(data.frame(Message="Run LD to populate pairs."),
                             options=list(dom="t"), rownames=FALSE))
      }
      ...
      top <- lp2 %>%
        dplyr::left_join(fl_small %>% dplyr::rename(i = ix, SNP_i = SNP, BP_i = BP, cand_i = is_candidate, class_i = classe),
                         by = "i", relationship = "many-to-one") %>%
        dplyr::left_join(fl_small %>% dplyr::rename(j = ix, SNP_j = SNP, BP_j = BP, cand_j = is_candidate, class_j = classe),
                         by = "j", relationship = "many-to-one")
      
      DT::datatable(top, rownames = FALSE, options = list(pageLength = 12, scrollX = TRUE))
    })

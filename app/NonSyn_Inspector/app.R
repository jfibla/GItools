  ...
  observeEvent(input$run_nonsyn_clusters, {
    ...
    tryCatch({
      ...
      tryCatch({
        ...
        get_nonsyn_gwas_path <- function(input) {
    ...
    settings_list = NULL
        ...
        current_cluster_method <- if (hub_mode_now) {
          as.character(st_sync$cluster_method %||% "unknown")
        } else {
          get_nonsyn_cluster_method(input)
        }
        
        current_threshold <- if (hub_mode_now) {
          suppressWarnings(as.numeric(st_sync$thr_value %||% NA_real_))
        } else {
          get_nonsyn_threshold(input, current_cluster_method)
        }
        
        current_gwas_name <- get_nonsyn_gwas_name(input)
        current_gwas_path <- get_nonsyn_gwas_path(input)
        ...
        if (!identical(
          ...
          stop("Existing manifest cluster_method does not match canonical cluster_method.")
        }

...
####### TEMPORAL ############
cat("\n================ GI PATH DEBUG ================\n")
app_name <- if (exists("APP_KEY")) APP_KEY else "gtex"
cat("APP:", app_name, "\n")
cat("PID:", Sys.getpid(), "\n")
cat("WD:", getwd(), "\n")
  ...
  observeEvent(input$run_gtex, {
    ...
    tryCatch({
      ...
      tryCatch({
        ...
        get_gtex_gwas_path <- function(input) {
    ...
    settings_list = NULL
        ...
        current_threshold <- if (hub_mode_now) {
          ...
          get_gtex_threshold(input, current_cluster_method)
        }
        
        current_gwas_name <- get_gtex_gwas_name(input)
        current_gwas_path <- get_gtex_gwas_path(input)
        ...
        canonical_settings_safe <- tryCatch({
          ...
          cat("[GTEX] canonical settings from gi_sync$state_shared() failed:", conditionMessage(e), "\n")
          NULL
        })
